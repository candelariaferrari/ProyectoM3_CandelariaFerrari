...
- **Migración al SDK oficial de Gemini:** después de la clase consulta donde el proyecto consumía la API con `fetch` nativo, el profesor pidió explícitamente usar el SDK (`@google/generative-ai`) y no tenía muy claro por dónde arrancar ese cambio. Le pedí que me explicara primero qué diferencias había entre lo que ya tenía armado y el SDK (dejar de construir el `fetch` a mano, usar `GoogleGenerativeAI`/`getGenerativeModel`/`generateContent`, y que la respuesta y los errores llegan de otra forma que una llamada REST cruda), y a partir de esa explicación lo fuimos armando juntos: la función que convierte el historial interno al formato `contents` que pide el SDK, y cómo adaptar la detección de rate-limit (429) a la forma en que el SDK reporta errores. También actualizamos los tests que dependían de la forma vieja de la respuesta. Me sirvió para entender las dos formas de integrar una API externa (a mano con fetch vs. con el SDK del proveedor) y cuándo tiene sentido usar cada una. 
